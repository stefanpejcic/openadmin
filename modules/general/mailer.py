...
# * Version: 1.6.0                                                        *
# * Build Date: 2025-09-23 11:29:35                                       *
# *                                                                       *
# *************************************************************************
# *                                                                       *
...
def save_email_to_file(subject, recipient, body):
    ...
    print(f"MAILER - Saving email to: {filepath}")    
...
def get_public_ip():
    print(f"MAILER - Checking public IPv4 address..")    
    try:
        ...
        print("MAILER - Failed to retrieve public IP:", e)
        ...
        try:
            ...
            print("MAILER - Failed to retrieve local IP:", e)
            return None
...
def load_openpanel_config(config_file_path):
    print(f"MAILER - Reading: {config_file_path}")    
    config_data = {}
...
def validate_one_time_code(one_time_code):
    print(f"MAILER - Validating one-time token: {one_time_code}")    
    config_file_path = '/etc/openpanel/openpanel/conf/openpanel.config'
    config_data = load_openpanel_config(config_file_path)
...
def get_cpu_usage():
    print(f"MAILER - Checking CPU usage..")    
    try:
        cpu_usage = psutil.cpu_percent(interval=1)
        return cpu_usage
    except Exception as e:
        print(f"MAILER - An unexpected error occurred: {e}")
        return None

def get_disk_usage():
    print(f"MAILER - Checking server disk usage..")
    try:
...
def get_ram_usage():
    print(f"MAILER - Checking server RAM usage..")
    try:
...
def count_crons_executed_today():
    print(f"MAILER - Checking number of cronjobs executed today..")
...
def count_api_requests_received_today():
    print(f"MAILER - Checking number of API requets executed today..")
...
def smtp_mailer(app):
    ...
    default_config = {
        'mail_server': 'mail.openpanel.com',
        'mail_port': 465,
        'mail_use_ssl': True,
        'mail_username': 'no-reply@openpanel.org',
        'mail_password': '96e2ygBDrThxl9zI',
        'mail_default_sender': 'no-reply@openpanel.org',
        'mail_debug': False,
    }
    ...
    app.config['MAIL_DEBUG'] = str(config_data.get('SMTP', {}).get('mail_debug')).lower() == 'true'
    ...
    def send_email():
        if request.method == 'POST':
            ...
            if 'Daily Usage Report' in message_content:
                print(f"MAILER - Generating Daily Usage Report..")
            ...
            # EMAIL SENT TO USER IF ADMIN SELECTED TO SEND THEM LOGINS!
            elif 'OpenPanel URL' in message_content:
                print(f"MAILER - Sending email to new user account with login information..")
                email_template = render_template('system/new_user_email.html', title=subject, message=message_content, hostname=server_hostname)
            # EMAIL FOR USER TRIGGERED BY NOTIFICATIONS MODULE ON USER PANEL
            elif 'changed for account' in message_content or  "login from" in message_content:
                print(f"MAILER - Sending notification email to user account..")
            ...
            # EVERYTHING ELSE IS FOR ADMINS!
            else:
                print(f"MAILER - Sending email to Administrator..")
            ...
            try:
                message = Message(subject=message_title, recipients=[recipient], html=email_template)
                ...
                if dev_mode.lower() == 'on':
                    error_info = {
                        ...
                        "mail_debug": app.config['MAIL_DEBUG'],
                        "mail_username": app.config['MAIL_USERNAME'],
                        #"mail_password": app.config['MAIL_PASSWORD'], #should not be shown, ever!
                        "mail_default_sender": app.config['MAIL_DEFAULT_SENDER']
                    }
                    print(f"MAILER - Email sending failed: {error_info}")                    
                    return jsonify(error_info), 500  
                else:
                    return jsonify({"error": f"An error occurred: {str(e)}"}), 500
                    print(f"MAILER - Email sending errored: {str(e)}")                    
