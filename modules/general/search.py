...
try:
    ...
    print(f"SEARCH - Error: File '{json_file_path}' not found.")

@app.route('/search/pages')
@admin_required_route
def search_filter():
    limit_results = 100
    if os.path.exists(json_file_path):
        print(f"SEARCH - Reading: {json_file_path}")
        try:
            with open(json_file_path, 'r') as json_file:
                data = json.load(json_file)
                return jsonify(data[:limit_results])
        except Exception as e:
            print(f"SEARCH - Error reading JSON data from file.")
...
def get_domain_owner(domain_name):
    ...
    try:
        ...
        print(f"SEARCH - Executing: opencli domains-whoowns {domain_name}")
        if "not found in the database" in output:
            print(f"SEARCH - Domain {domain_name} does not exist.")
            ...
            print(f"SEARCH - Domain {domain_name} is owned by user: {username}")
            if 'output' in request.args and request.args['output'] == 'json':
        ...
        # Log the error
        print(f"SEARCH - Error executing command: {e}")
...
def get_all_websites():
    ...
    cursor = conn.cursor()
    print(f"SEARCH - Fetching all websites from database.. (data is cached for 300s)")
    select_data_query = """
    SELECT site_name
    FROM sites
    """
...
@app.route('/search/websites', defaults={'site_name': None})
@app.route('/search/websites/<site_name>')
@admin_required_route
def search_websites(site_name):
    if site_name:
        ...
        LIMIT 5
        """
        print(f"SEARCH - Executing query: {select_data_query}")
...
def get_all_users():
    ...
    FROM users
    """
    print(f"SEARCH - Fetching all users from database.. (data is cached for 300s)")
    cursor.execute(select_data_query)
    data = cursor.fetchall()
    cursor.close()
    conn.close()
...
@app.route('/search/users', defaults={'username': None})
@app.route('/search/users/<username>')
@admin_required_route
def search_users(username):
    if username:
        ...
        LIMIT 5
        """
        print(f"Executing query: {select_data_query}")
