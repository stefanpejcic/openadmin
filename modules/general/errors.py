...
# import our modules
from app import app, load_openpanel_config, login_required_route

# logs
log_directories = ['/var/log/openpanel/admin', '/var/log/openpanel/user']
for log_directory in log_directories:
    if not os.path.exists(log_directory):
        print(f"ERRORS - Creating directory: {log_directory}")
...
# blocks 404 on /static/something
@login_required_route
def protected_error_page():
...
def custom_404_error_handler(error):
    if request.path.startswith('/api/'):
        ...
        if api_status_current_value.lower() == 'on':
            print(f"ERRORS - This api route does not exist. Please check the documentation: https://dev.openpanel.com/openadmin-api/")
            return jsonify({'error': 'This api route does not exist. Please check the documentation: https://dev.openpanel.com/openadmin-api/'}), 404
        else:
            print(f"ERRORS - API access is disabled! To enable api access: opencli config update api on")
            return jsonify({'error': 'API access is disabled! To enable api access OpenAdmin > Settings'}), 404

    return protected_error_page()
...
def handle_500(error):
    ...
    return jsonify({"error": "System error", "message": error}), 500

@app.errorhandler(CSRFError)
def handle_csrf_error(error):
    print(f"ERRORS - CSRF error: {error.description}")    
...
def handle_exception(error):
    ...
    print(f"ERRORS - Unhandled exception: {error}")    
    message = str(error) if str(error) else "An unexpected error occurred"
    return render_error_template(message, 500)
