...
tee $generated_report &>/dev/null <<EOF
$break
 GENERAL INFO
$break
...
$break
This report was generated by the Sentinel service on OpenPanel server $hostname at $formatted_date.

report file: $generated_report
$break
EOF
}
...
check_system_load() {
  ...
  if [ "$load_int" -gt "$LOAD_THRESHOLD" ]; then
    ...
    echo -e "\e[31m[✘]\e[0m Average Load usage ($load_int) is higher than treshold value ($LOAD_THRESHOLD). Generating crash-report and writing notification."
